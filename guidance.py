...
class Guide():
    def __init__(self,
        ...
        self.device = device
        self.encoder = CLIPEncoder(clip, tokenizer)
        # Placeholder embed is used for its header token in direct image
        #   guidance
        self.placeholder_embed = self.encoder.prompt('{}')
    ...
    def embeds(self,
        ...
        if prompt:
            ...
            text_embeddings = self.encoder.prompt(prompt)
        if guide is not None:
            if isinstance(guide, str):
                guide = guide.strip()
                if guide:
                    guide_embeddings = self.encoder.prompt(guide)
            else:
                guide_embeddings = self.encoder.image(guide)
                if mapping_concepts:
                    concept_mapper = ConceptMapper(
                        guide_embeddings, self.encoder.prompt(mapping_concepts))
