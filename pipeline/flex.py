...
class FlexPipeline(DiffusionPipeline):
    ...
    def __call__(self,
        ...
        batch_size = guide.batch_size

        # set timesteps
        self.scheduler.set_timesteps(guide.steps)
        ...
        if init_image:
            ...
            init_timestep = int(guide.steps * strength) + offset
            init_timestep = min(init_timestep, guide.steps)
            if isinstance(self.scheduler, LMSDiscreteScheduler):
                timesteps = torch.tensor([guide.steps - init_timestep]
                                         * batch_size,
                                         dtype=torch.long,
                                         device=self.device)
            else:
            ...
            # Calc start timestep
            t_start = max(guide.steps - init_timestep + offset, 0)
        else:
        ...
        # TODO: Training-Free Structured Diffusion:
        # https://openreview.net/pdf?id=PUIqjT4rzq7
        # Method that modifies the cross attention layers of the model to
        #   improve "compositionality". It does this through an algorithmic
        #   process...
        for i, t in enumerate(
            ...
            noise_pred = guide.noise_pred(latent_model_input, t)
